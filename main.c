...
#define	Kgf  3.24447  	//geometric factor		// for Sr 101 to 105 diagonal sonic beam type sensor	 
//#define	Kgf  2.83892  	//geometric factor		//    for Sr 106 for helical sonic beam type sensor

#define Kh   0.93120  	//hydraulic constant	   // for Sr 101 to 105 diagonal sonic beam type sensor
//#define Kh   1.00000  			                   //    for Sr 106 for helical sonic beam type sensor

#define Alpha   1.0000  //Calibration constant 
//ch-1 = 1.0100,    ch-2 = 1.0100 ,   ch-3 = 1.0100		 as ON:05-12-14 after calibration at IDEMI for sr-105
//ch-1 = 1.0400,    ch-2 = 1.0400 ,   ch-3 = 1.0400		 as ON:05-12-14 after calibration at IDEMI for sr-102 (alphas re adjusted)
//ch-1 = 1.0400,    ch-2 = 1.0400 ,   ch-3 = 1.0500		 as ON:05-12-14 after calibration at IDEMI for sr-102
//ch-1 = 0.9737,    ch-2 = 1.0101 ,   ch-3 = 0.9940		 as ON:06-03-14 after calibration at IDEMI for sr-101
//ch-1 = 1.000,    ch-2 = 1.000 ,   ch-3 = 1.000		 as ON:03-12-14 after calibration at IDEMI for sr-103
//ch-1 = 1.000,    ch-2 = 1.000 ,   ch-3 = 1.000		 as ON:04-12-14 after calibration at IDEMI for sr-104
//ch-1 = 1.003,    ch-2 = 1.000 ,   ch-3 = 1.013		 as ON:10-12-14 after calibration at IDEMI for sr-106

#define AVTCORRECTION    0 // ddp 040314 //avarage T Correction
#define	MKfact1	((Kgf * Kh * Alpha)* 262144000) 				 //*DDP*//
#define	MKfact2	1000

// TDC 1 count = 3.814699 pS
...
#define TDC_100US 26148281// ddp 040314//it is temp data actual data is = 26214400//25984026//25974026//tdc count coresponding to 100us 25000 ?	 *****	 //*DDP*//
...
//				statusLedBlink = 0;						  /////##### DDP
			...
			else if( (operMode == measureMode)&& (faultBits == 1) )//any fault
			{
				statusLedBlink = 0;
				statusLedOff;
...
//					faultBits &= ~2;
			...
			tof =  ((flowCalc.avgTr+flowCalc.avgTf)*3.814/(2000000));
//			delT = 3.814*(float)(flowCalc.avgTr-flowCalc.avgTf-avgTcorr)/1000;
//               printf("%d \t %d\r\n",flowCalc.avgTf,TDC_100US);
//			printf("%d  %d  %d \r\n",TDC_status_f,TDC_status_r,(faultCountF+faultCountR));			
			printf("%4.2f  %4.2f  %d  %4.5f  %d %d %d %02X\r\n",((finalFlowOutput*63)/3900),flow1Sec.flow,(flowCalc.avgTr-flowCalc.avgTf-avgTcorr),tof,avgTcorr,faultCountF,faultCountR,displayvalue);
...
//		W3LO;		//w3 LOW				                 // ***$$$$DDP
	...
	VICIntEnClr  = 0xffffffff; /* disable all interrupt     */  //*sipl  DDP040314//
	...
	W3HI;		//w3 High						                 // ***$$$$DDP
	VICIntEnable = (1<<PWM0_1_INT)|(1<<UART2_INT); /* Enable Interrupt     */  //*sipl    DDP040314
...
float calcFlow(struct fC *flow) //calculates flow from time diff of TDC
{
  ...
  Tx =(Tx - T3RDHIT - T2STEEL)/6.2426;	    //    for Sr 101 to 105 diagonal sonic beam type sensor
	// Tx =(Tx - T3RDHIT - T2STEEL)/5.9686;		//    for Sr 106 for helical sonic beam type sensor
