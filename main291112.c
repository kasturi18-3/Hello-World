...
#define Alpha   0.8924  	//Calibration constant					  //*DDP*//
...
#define	MKfact2	1000
#define T2STEEL	413663		   //eqnt of 2*0.789uS =1.578uS  steel sound velo 5700m/s and thickness 4.5mm
#define T3RDHIT	668390   		   //eqnt of 2.55 uS
...
{	
	...
	int faultBits,relayk2disturb;
	float deltaFlow,deltaFlow_temp;
	int i,j=0,k=0;
...
/////////////////////////DDP--start////////////////////////////////////////////
			...
			SPIBuff[0] = 0xB1; 		//read 2nd hit data
			spTxRx(SPIBuff,5);
			TDC_reverse_2 = ((SPIBuff[1]<<8) | SPIBuff[2]);
			TDC_reverse_2 = (TDC_reverse_2<<16)|((SPIBuff[3]<<8) | SPIBuff[4]);

 			SPIBuff[0] = 0xB2; 		//read 2nd hit data
			spTxRx(SPIBuff,5);
			TDC_reverse_3 = ((SPIBuff[1]<<8) | SPIBuff[2]);
			TDC_reverse_3 = (TDC_reverse_3<<16)|((SPIBuff[3]<<8) | SPIBuff[4]);
...
/////////////////////////DDP--start////////////////////////////////////////////
			...
			SPIBuff[0] = 0xB1; 		//read 2nd hit data
			spTxRx(SPIBuff,5);
			TDC_forward_2 = ((SPIBuff[1]<<8) | SPIBuff[2]);
			TDC_forward_2 = (TDC_forward_2<<16)|((SPIBuff[3]<<8) | SPIBuff[4]);

 			SPIBuff[0] = 0xB2; 		//read 2nd hit data
			spTxRx(SPIBuff,5);
			TDC_forward_3 = ((SPIBuff[1]<<8) | SPIBuff[2]);
			TDC_forward_3 = (TDC_forward_3<<16)|((SPIBuff[3]<<8) | SPIBuff[4]);

//printf("6\n");

			if( index_1 >= 16)
				index_1 = 0;
				Diff_Rv_32_1sec[index_1] = (TDC_reverse_3-TDC_reverse_2);
				Diff_Rv_21_1sec[index_1] = (TDC_reverse_2-Cur_val.TdcRv);
				Diff_Fr_32_1sec[index_1] = (TDC_forward_3-TDC_forward_2);
				Diff_Fr_21_1sec[index_1] = (TDC_forward_2-Cur_val.TdcFr);
				index_1 = index_1 + 1;

 				memcpy(&bufdummy[0],&Diff_Rv_32_1sec[0],16*4);//copy 16 int to temp
				quickSort(bufdummy, 16);
				for(i = 4,sum = 0; i < 12; i++)
					sum += bufdummy[i];
				Diff_Rv_32_temp = (sum >> 3);
//printf("7\n");

 				memcpy(&bufdummy[0],&Diff_Rv_21_1sec[0],16*4);//copy 16 int to temp
				quickSort(bufdummy, 16);
				for(i = 4,sum = 0; i < 12; i++)
					sum += bufdummy[i];
				Diff_Rv_21_temp = (sum >> 3);
//printf("8\n");

  				memcpy(&bufdummy[0],&Diff_Fr_32_1sec[0],16*4);//copy 16 int to temp
				quickSort(bufdummy, 16);
				for(i = 4,sum = 0; i < 12; i++)
					sum += bufdummy[i];
				Diff_Fr_32_temp = (sum >> 3);
//printf("9\n");

 				memcpy(&bufdummy[0],&Diff_Fr_21_1sec[0],16*4);//copy 16 int to temp
				quickSort(bufdummy, 16);
				for(i = 4,sum = 0; i < 12; i++)
					sum += bufdummy[i];
				Diff_Fr_21_temp = (sum >> 3);

//printf("10\n");
				if( index_5 >= BUFFLONGFILTER)
				index_5 = 0;

				Diff_Rv_32_5sec[index_5] = Diff_Rv_32_temp;
				Diff_Rv_21_5sec[index_5] = Diff_Rv_21_temp;
				Diff_Fr_32_5sec[index_5] = Diff_Fr_32_temp;
				Diff_Fr_21_5sec[index_5] = Diff_Fr_21_temp;
				index_5 = index_5 + 1;

				memcpy(&bufdummy[0],&Diff_Rv_32_5sec[0],160*4);//copy 160 int to temp
				quickSort(bufdummy, 160);
				for(i = 16,sum = 0; i < BUFFLONGFILTER-16; i++)
					sum += bufdummy[i];
				Diff_Rv_32_temp = (sum >> 7);
				Diff_Rv_32 =   0.994*Diff_Rv_32 + 0.006*Diff_Rv_32_temp;		//%%%%%*****$$$$$  DDP
//printf("11\n");
				memcpy(&bufdummy[0],&Diff_Rv_21_5sec[0],160*4);//copy 160 int to temp
				quickSort(bufdummy, 160);
				for(i = 16,sum = 0; i < BUFFLONGFILTER-16; i++)
					sum += bufdummy[i];
				Diff_Rv_21_temp = (sum >> 7);
				Diff_Rv_21 =   0.994*Diff_Rv_21 + 0.006*Diff_Rv_21_temp;		//%%%%%*****$$$$$  DDP
//printf("12\n");
				memcpy(&bufdummy[0],&Diff_Fr_32_5sec[0],160*4);//copy 160 int to temp
				quickSort(bufdummy, 160);
				for(i = 16,sum = 0; i < BUFFLONGFILTER-16; i++)
					sum += bufdummy[i];
				Diff_Fr_32_temp = (sum >> 7);
				Diff_Fr_32 =   0.994*Diff_Fr_32 + 0.006*Diff_Fr_32_temp;		//%%%%%*****$$$$$  DDP
//printf("13\n");
				memcpy(&bufdummy[0],&Diff_Fr_21_5sec[0],160*4);//copy 160 int to temp
				quickSort(bufdummy, 160);
				for(i = 16,sum = 0; i < BUFFLONGFILTER-16; i++)
					sum += bufdummy[i];
				Diff_Fr_21_temp = (sum >> 7);
...
//			printf("%d  %d  %d \r\n",TDC_status_f,TDC_status_r,(faultCountF+faultCountR));			
			printf("%4.2f  %4.2f  %d  %d  %d  %d  %d  %d  %d\r\n",((finalFlowOutput*63)/3900),flow1Sec.flow,(flowCalc.avgTr-flowCalc.avgTf-avgTcorr),(Diff_Rv_32+Diff_Rv_21-Diff_Fr_32-Diff_Fr_21),Diff_Rv_32,Diff_Rv_21,Diff_Fr_32,Diff_Fr_21,avgTcorr);
